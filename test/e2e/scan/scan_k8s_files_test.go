	...
	Context("scan iac files violating k8s policies", func() {
		...
		Context("iac type is specified as k8s", func() {
			It("should scan and display violations in human output format", func() {
				...
				scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_human.txt", 3, false, true, outWriter, errWriter, scanArgs...)
			})
			...
			When("-v flag is used for verbose output", func() {
				It("should display verbose output for human output format", func() {
					...
					scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_human_verbose.txt", 3, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is json", func() {
				It("should display violations in json format", func() {
					...
					scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_json.txt", 3, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is yaml", func() {
				It("should display violations in yaml format", func() {
					...
					scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_yaml.txt", 3, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is xml", func() {
				It("should display violations in xml format", func() {
					...
					scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_xml.txt", 3, false, true, outWriter, errWriter, scanArgs...)
				})
			})
			...
			When("when output type is junit-xml", func() {
				It("should display violations in junit-xml format", func() {
					...
					scanUtils.RunScanCommandAndAssertTextOutput(terrascanBinaryPath, "golden/k8s_scans/k8s/kubernetes_ingress_violations/kubernetes_ingress_junit_xml.txt", 3, true, true, outWriter, errWriter, scanArgs...)
				})
			})
		})
	})
