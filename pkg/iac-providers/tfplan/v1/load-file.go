...
func (t *TFPlan) LoadIacFile(absFilePath string, options map[string]interface{}) (allResourcesConfig output.AllResourceConfigs, err error) {
	...
	if err != nil {
		...
		zap.S().Debug(errMsg)
		return allResourcesConfig, fmt.Errorf("failed to read tfplan JSON file. error: '%v'", err) //lint:ignore SA1006 placeholder %s are specified in string constants
	}
	...
	if err != nil {
		...
		zap.S().Debug(errMsg)
		return allResourcesConfig, fmt.Errorf("failed to process tfplan JSON. error: '%v'", err) //lint:ignore SA1006 placeholder %s are specified in string constants
	}
	...
	if err := json.Unmarshal(processed, &resourceConfigs); err != nil {
		...
		zap.S().Debug(errMsg)
		return allResourcesConfig, fmt.Errorf("failed to decode processed jq output. error: '%v'", err) //lint:ignore SA1006 placeholder %s are specified in string constants
	}
