...
func LoadIacFile(absFilePath, terraformVersion string) (allResourcesConfig output.AllResourceConfigs, err error) {
	...
	if hclFile == nil {
		...
		return allResourcesConfig, fmt.Errorf("error occurred while loading config file '%s'. error:\n%v\n", absFilePath, getErrorMessagesFromDiagnostics(diags)) //lint:ignore SA1006 placeholder %s are specified in string constants
	}

	if diags.HasErrors() {
		errMessage := fmt.Sprintf("failed to load iac file '%s'. error:\n%v\n", absFilePath, getErrorMessagesFromDiagnostics(diags))
		zap.S().Debug(errMessage)
		return allResourcesConfig, fmt.Errorf("failed to load iac file '%s'. error:\n%v\n", absFilePath, getErrorMessagesFromDiagnostics(diags)) //lint:ignore SA1006 placeholder %s are specified in string constants
	}
