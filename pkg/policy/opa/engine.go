...
// ReportVulnerability Add a vulnrability for a given resource
func (e *Engine) ReportVulnerability(engineInput policy.EngineInput) policy.EngineOutput {
	for _, resources := range *engineInput.InputData {
		for _, resource := range resources {
			for _, container := range resource.ContainerImages {
				vulnerabilities := prepareVulnerabilityObject(container, resource)
				e.results.ViolationStore.Vulnerabilities =
					append(e.results.ViolationStore.Vulnerabilities, vulnerabilities...)
	...
	e.results.Summary.Vulnerabilities = len(e.results.ViolationStore.Vulnerabilities)
	return e.results
}

// prepareVulnerabilityObject Add a vulnrability for a given resource
func prepareVulnerabilityObject(container output.ContainerNameAndImage, resource output.ResourceConfig) []*results.Vulnerability {
