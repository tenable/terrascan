...
const arm_networkInterfaces = "networkInterfaces"
const tf_networkInterfaceIDs = "network_interface_ids"
...
func VirtualMachineConfig(r types.Resource, params map[string]interface{}) map[string]interface{} {
	...
	profile := convert.ToMap(r.Properties, arm_networkProfile)
	if interfaces, ok := profile[arm_networkInterfaces].([]interface{}); ok {
		iFaceIDs := []string{}
		for _, fs := range interfaces {
			iFace := fs.(map[string]interface{})
			iFaceIDs = append(iFaceIDs, iFace["id"].(string))
		}
		cf[tf_networkInterfaceIDs] = iFaceIDs
	}
	return cf
}
