...
checkConfiguration(sg) {
    ...
    checkScopeIsPrivate(sg.source_address_prefix)
    checkPort(sg, "{{.portNumber}}")
    checkProtocol(sg.protocol, "{{.protocol}}")
}

checkScopeIsPrivate(source_address_prefix) {
    ...
    hosts = split(source_address_prefix, "/")
    to_number(hosts[1]) < 27
}


checkPort(config, portNumber) {
    config.destination_port_range == portNumber
}

checkPort(config, portNumber) {
    config.source_port_range == portNumber
}
