...
# This script is taken from https://github.com/kubernetes-sigs/kustomize/tree/master/hack and has been modified 
# in order to support non module binaries.
...
if [ -n "$1" ]; then
  if [[ "$1" =~ ^[0-9]+(\.[0-9]+){2}$ ]]; then
    getval=$(getTagName $1)
    release_url=${release_url}/tags/$getval
  elif [ -n "$2" ]; then
